...
namespace SpacePlanning
{
    public static class SpacePlanning
    {
        ...
        public static SpacePlanningOutputs Execute(Dictionary<string, Model> inputModels, SpacePlanningInputs input)
        {
            ...
            AssignSpaceToAutogeneratedSpaces(input.Overrides.Spaces, spaces);
        ...
        private static void AssignSpaceToAutogeneratedSpaces(IList<SpacesOverride> overrideSpaces, List<SpaceBoundary> cutSpaces)
        {
            foreach (var overrideSpace in overrideSpaces)
            {
                var identity = (SpacesIdentity)overrideSpace.GetIdentity();

                foreach (var cutSpace in cutSpaces)
                {
                    if (cutSpace.ProgramType == "unspecified" && ((identity.OriginalBoundary.Contains(cutSpace.Boundary.Perimeter.Centroid()))))
                    {
                        cutSpace.SetProgram(overrideSpace.Value.ProgramType);
                    }
                }
            }
        }
